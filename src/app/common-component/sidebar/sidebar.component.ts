...
export class SidebarComponent {
  ...
  public sidebarDataDoc: Array<SideBarData> = [];
  public has_subscription: boolean = false;
  ...
  constructor(
    ...
    if(this.user && this.user.role.includes('Super-Admin')){
      this.sidebarData = this.data.sideBar;
    }else{
      let SIDE_BAR_G:any = [];
      //Filtramos y validamos opciones del rol
      let permissionsUser = this.user.permissions;
      this.data.sideBar.forEach((side:any) => {
        let SIDE_B:any = [];
        side.menu.forEach( (menu_s:any) => {
          let SUB_MENUS = menu_s.subMenus.filter((submenu:any) => permissionsUser.includes(submenu.permision) && submenu.show_nav == true); 
          if(SUB_MENUS.length > 0){
           menu_s.subMenus = SUB_MENUS;
           SIDE_B.push(menu_s);
          }else{
            if(permissionsUser.includes(menu_s.permision)){
              SIDE_B.push(menu_s);
            }
          }
        })
