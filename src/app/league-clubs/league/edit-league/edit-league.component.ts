...
export class EditLeagueComponent {
  ...
  constructor( public leagueSrv: LeagueService, public activateRoute: ActivatedRoute, public categorySrv: CategoryService, 
  ...
  getLeagueSelected(){
    this.leagueSrv.getLeague(this.id_league).subscribe( (resp:any) => {
      ...
      this.categories_list = this.league_selected.category;
      ...
      this.match_type_id = this.league_selected.type_match;
    });
  }
  ...
  save(){
    ...
    this.leagueSrv.editLeague(this.id_league, formData).subscribe( (resp: any) => {
      if( resp.message == 200){
