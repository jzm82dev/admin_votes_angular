...
export class PlayersComponent {
  ...
  public routes = routes;
    ...
    getTournamentplayers(){
      this.tournamentSrv.getAllPlayersTournament( this.tournament_id).subscribe( (resp:any) => {
   ...
   private getTableData(page = 1, namePlayer = '',  date = '', category_id = ''): void {
    ...
    this.tournamentSrv.getAllPlayersTournament(this.tournament_id, page, namePlayer, this.payment_status, this.court_value, date_match, this.category_id).subscribe((resp: any) => {
  ...
  paidTournament(player_id: string){
    this.tournamentSrv.paidPlayerTournament(player_id).subscribe( (resp:any) => {
      if( resp.message == '200'){
  ...
  unpaidTournament(player_id: string){
    this.tournamentSrv.unpaidPlayerTournament(player_id).subscribe( (resp:any) => {
      if( resp.message == '200'){
        let index = this.playersList.findIndex((item:any) => item.id == player_id);
        if(index != -1){
