...
export class ListMemberComponent {
 ...
 private modalError: any;
  ...
  constructor(public data : DataService, public memberSrv: MembersService, public translate: TranslateService){
  }
  ngOnInit() {
  ...
  private getTableData(): void {
    ...
    this.memberSrv.listMembers().subscribe((resp: any) => {
      this.totalData = resp.members.length;
      this.members_general = resp.members;
 ...
 hasPermission( permision: string){
  ...
  getTableDataGeneral() {
    ...
    this.members_general.map((res: any, index: number) => {
  ...
  public searchData(value: any): void {
    ...
    this.membersList = this.dataSource.filteredData;
  }
  ...
  selectMember( member:any){
    this.member_selected = member;
  }

  removeMember(){
      this.memberSrv.deleteMember(this.member_selected.user_id).subscribe( (resp:any) => {
        if( resp.message == 200){
          let index = this.membersList.findIndex((item:any) => item.user_id == this.member_selected.user_id);
          if(index != -1){
